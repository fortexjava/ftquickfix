...
public abstract class AbstractIoHandler extends IoHandlerAdapter {
	...
	protected final Logger log = LoggerFactory.getLogger("Event");
    ...
    public void sessionClosed(IoSession ioSession) throws Exception {
        try {
            ...
            ioSession.close(true);
            //Patrick Chi 
            //disconnect session if session is closed, fix issue for sometimes multipul logon.
            if(quickFixSession != null && ioSession != null)
            	quickFixSession.disconnect("Socket exception ("  + ioSession.getRemoteAddress() + ") ", true);
        } catch (Exception e) {
            ioSession.close(true);
            throw e;
        }
    ...
    public void messageReceived(IoSession ioSession, Object message) throws Exception {
    	//Patrick update for current time
        ...
        if (quickFixSession != null) {
            try {
                ...
                quickFixSession.getLog().onIncoming(messageString, fixMessage.getHeader().getString(MsgType.FIELD));
                processMessage(ioSession, fixMessage);
                
            } catch (InvalidMessage e) {
            	//Patrick update for logging
            	 quickFixSession.getLog().onIncoming(messageString, null);
