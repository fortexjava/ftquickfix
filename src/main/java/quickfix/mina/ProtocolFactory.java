...
public class ProtocolFactory {
    ...
    public static IoAcceptor createIoAcceptor(int transportType) {
        if (transportType == SOCKET) {
            ...
            //////Add Thread Pool for acceptor	By Ivan
            //ret.getFilterChain().addLast("threadPool", new ExecutorFilter(Executors.newCachedThreadPool()));
            //ret.getFilterChain().addLast("threadPool", new ExecutorFilter(8));
    ...
    public static IoConnector createIoConnector(SocketAddress address) throws ConfigError {
        if (address instanceof InetSocketAddress) {
        	NioSocketConnector con = new NioSocketConnector();
        	  //////Add Thread Pool for acceptor	By Ivan
        	//con.getFilterChain().addLast("executor", new ExecutorFilter(Executors.newCachedThreadPool()));
        	//con.getFilterChain().addLast("threadPool", new ExecutorFilter(8));
        	return con;
        } else if (address instanceof VmPipeAddress) {
