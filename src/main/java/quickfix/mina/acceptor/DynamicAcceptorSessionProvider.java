...
public class DynamicAcceptorSessionProvider implements AcceptorSessionProvider {
    ...
    protected SessionID lookupTemplateID(SessionID sessionID) throws ConfigError{
        for (TemplateMapping mapping : templateMappings) {
        	//Update by patrick
            //if (isMatching(mapping.getPattern(), sessionID)) {
        	isMatching(mapping.getPattern(), sessionID); 
    ...
    //private boolean isMatching(SessionID pattern, SessionID sessionID) throws ConfigError{
    //Update by patrick
    private void isMatching(SessionID pattern, SessionID sessionID) throws ConfigError{
    	if (!isMatching(pattern.getBeginString(), sessionID.getBeginString())) {
    		throw new ConfigError("Invalid BeginString in sessionID:" + sessionID);
    	} else if (!isMatching(pattern.getSenderCompID(), sessionID.getSenderCompID())) {
    		throw new ConfigError("Invalid SenderCompID in sessionID:" + sessionID);
    	} else if (!isMatching(pattern.getSenderSubID(), sessionID.getSenderSubID())) {
    		throw new ConfigError("Invalid SenderSubID in sessionID:" + sessionID);
    	} else if (!isMatching(pattern.getSenderLocationID(), sessionID.getSenderLocationID())) {
    		throw new ConfigError("Invalid SenderLocationID in sessionID:" + sessionID);
    	} else if (!isMatching(pattern.getTargetCompID(), sessionID.getTargetCompID())) {
    		throw new ConfigError("Invalid TargetCompID in sessionID:" + sessionID);
    	} else if (!isMatching(pattern.getTargetSubID(), sessionID.getTargetSubID())) {
    		throw new ConfigError("Invalid TargetSubID in sessionID:" + sessionID);
    	} else if (!isMatching(pattern.getTargetLocationID(), sessionID.getTargetLocationID())) {
    		throw new ConfigError("Invalid getTargetLocationID in sessionID:" + sessionID);
    	}
